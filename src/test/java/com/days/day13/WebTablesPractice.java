...
public class WebTablesPractice {
    ...
    public void test2() {
        ...
        int actualColumnNumber = driver.findElements(By.xpath("//table[@id='table1']//th")).size();  // header location
        int expectedColumnNumber = 6;
        Assert.assertEquals(actualColumnNumber, expectedColumnNumber);
    }
    //to exclude first row (thead rows) = //table[@id='table1']//tbody//tr (5-1=4 rows)  -->same location and add tbody
    // (5-1=4 rows) --> because first row is for header
    ...
    public void test3() {
        ...
        int actualRowCount = driver.findElements(By.xpath("//table[@id='table1']//tr")).size();  // all rows
    ...
    public void test5() {
        ...
        //if index will exceed(gecmek, asmak) table size, you will not get any errors, list will be just empty
        //findElements() doesn't give NoSuchElementException, in any case.
        int index = 1;  // we use it to make this function for more dynamic
        List<WebElement> row = driver.findElements(By.xpath("//table[@id='table1']//tbody//tr[" + index + "]//td"));
        for (WebElement cell : row) {               //  css --> table[id='table1'] tbody tr:nth-of-type(2) td
            System.out.println(cell.getText());
        }
    }
    ...
    public void test6() {
        ...
        String expectedEmail = "jdoe@hotmail.com";                           // instead of  [3]               [3]
    ...
    public void test8() {
        ...
        for(WebElement lastName : lastNames){
            System.out.println(lastNames.getText());
            // in this way we can get the text but we can not compare the elements one by one
            // in this case we need to use for loop
         */
    }
    @AfterMethod
